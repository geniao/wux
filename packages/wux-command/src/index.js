...
const debug = _debug('wux-command:index')

export default class WuxCommand {
  constructor(rawArgv) {
    /**
     * original argument
     * @type {Array}
  ...
  add(name, target) {
    ...
    if (!(target.prototype instanceof WuxCommand)) {
      ...
      assert(
        target.prototype instanceof WuxCommand,
        'command class should be sub class of wux-command'
      )
    }
  ...
  errorHandler(err) {
    ...
    console.error(
      chalk.red('⚠️  Command Error, enable `DEBUG=wux-command` for detail')
    )
